...
mod tests {
    ...
    fn fixture_script_name(base: &str) -> String {
        if cfg!(windows) {
            format!("{base}.cmd")
        } else {
            format!("{base}.sh")
    ...
    fn write_plugin_fixture(root: &Path, name: &str, include_hooks: bool, include_lifecycle: bool) {
        ...
        if include_hooks {
            ...
            let pre_script = fixture_script_name("pre");
            let pre_path = root.join("hooks").join(&pre_script);
            #[cfg(windows)]
            let pre_contents = "@echo off\r\necho plugin pre hook\r\n";
            #[cfg(not(windows))]
            let pre_contents = "#!/bin/sh\nprintf 'plugin pre hook'\n";
            fs::write(&pre_path, pre_contents).expect("write hook");
            make_executable(&pre_path);
        }
        if include_lifecycle {
            fs::create_dir_all(root.join("lifecycle")).expect("lifecycle dir");
            let init_script = fixture_script_name("init");
            let shutdown_script = fixture_script_name("shutdown");
            let init_path = root.join("lifecycle").join(&init_script);
            let shutdown_path = root.join("lifecycle").join(&shutdown_script);

            #[cfg(windows)]
            let init_contents = "@echo off\r\necho init>> lifecycle.log\r\n";
            #[cfg(not(windows))]
            let init_contents = "#!/bin/sh\nprintf 'init\\n' >> lifecycle.log\n";
            fs::write(&init_path, init_contents).expect("write init lifecycle");
            make_executable(&init_path);

            #[cfg(windows)]
            let shutdown_contents = "@echo off\r\necho shutdown>> lifecycle.log\r\n";
            #[cfg(not(windows))]
            let shutdown_contents = "#!/bin/sh\nprintf 'shutdown\\n' >> lifecycle.log\n";
            fs::write(&shutdown_path, shutdown_contents).expect("write shutdown lifecycle");
        ...
        let lifecycle = if include_lifecycle {
            format!(
                ",\n  \"lifecycle\": {{\n    \"Init\": [\"./lifecycle/{}\"],\n    \"Shutdown\": [\"./lifecycle/{}\"]\n  }}",
                fixture_script_name("init"),
                fixture_script_name("shutdown")
            )
        } else {
            String::new()
        };
...
UU conflicted.rs",
    ...
    fn build_runtime_plugin_state_merges_plugin_hooks_into_runtime_features() {
        ...
        let expected_hook_suffix = format!("hooks/{}", fixture_script_name("pre"));
        assert!(
