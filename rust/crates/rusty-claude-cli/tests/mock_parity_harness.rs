...
fn run_case(case: ScenarioCase, workspace: &HarnessWorkspace, base_url: &str) -> ScenarioRun {
    ...
    preserve_windows_process_env(&mut command);
...
fn preserve_windows_process_env(command: &mut Command) {
    if cfg!(windows) {
...
fn prepare_plugin_fixture(workspace: &HarnessWorkspace) {
    ...
    #[cfg(windows)]
    {
        let python_path = tool_dir.join("echo-json.py");
        fs::write(
            &python_path,
            "import json, os, sys\npayload = json.loads(sys.stdin.read())\nprint(json.dumps({\"plugin\": os.environ.get(\"CLAWD_PLUGIN_ID\", \"\"), \"tool\": os.environ.get(\"CLAWD_TOOL_NAME\", \"\"), \"input\": payload}))\n",
        )
        .expect("plugin python script should write");
        fs::write(
            &script_path,
            "@echo off\r\npython \"%~dp0echo-json.py\"\r\n",
        )
        .expect("plugin cmd script should write");
    }
    #[cfg(not(windows))]
    {
        fs::write(
  ...
  "tools": [
    {
      ...
      "requiredPermission": "workspace-write"
        ...
        .replace("echo-json.sh", script_name),
    )
...
fn assert_read_file_roundtrip(_workspace: &HarnessWorkspace, run: &ScenarioRun) {
    ...
    let output = run.response["tool_results"][0]["output"]
        .as_str()
        .expect("tool output");
    assert!(normalize_path_separators(output).contains("fixture.txt"));
