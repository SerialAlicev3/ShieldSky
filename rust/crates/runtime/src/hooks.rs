...
fn parse_hook_output(stdout: &str) -> ParsedHookOutput {
    ...
    let hook_payload = match serde_json::from_str::<Value>(stdout) {
        Ok(payload) => payload,
        Err(_) if stdout.trim_start().starts_with("{\\\"") => {
            normalized_stdout = stdout.replace("\\\"", "\"");
            match serde_json::from_str::<Value>(&normalized_stdout) {
                Ok(payload) => payload,
                Err(_) => {
    ...
    let Value::Object(root) = hook_payload else {
        return ParsedHookOutput {
            messages: vec![stdout.to_string()],
            ..ParsedHookOutput::default()
        };
...
mod tests {
    ...
    #[cfg(windows)]
    fn windows_shell_snippet(script: &str) -> String {
        let trimmed = script.trim();
        if trimmed == "sleep 5" {
            return "ping -n 6 127.0.0.1 > NUL".to_string();
        }
        if let Some(rest) = trimmed.strip_prefix("printf '%s' '") {
            if let Some(json) = rest.strip_suffix('\'') {
                return format!("echo {json}");
            }
        }
        if let Some(rest) = trimmed.strip_prefix("printf '") {
            if let Some((message, tail)) = rest.split_once('\'') {
                let mut command = format!("echo {message}");
                if let Some(code) = tail.trim().strip_prefix("; exit ") {
                    command.push_str(" & exit /b ");
                    command.push_str(code);
                }
                return command;
            }
        }
