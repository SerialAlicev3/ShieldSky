#![allow(clippy::map_unwrap_or, clippy::needless_continue)]
...
fn detect_python_command() -> Option<&'static str> {
    #[cfg(windows)]
    {
        detect_first_command(&["python", "py", "python3"])
    }
    #[cfg(not(windows))]
    {
        detect_first_command(&["python3", "python"])
    }
}
...
mod tests {
    ...
    fn shell_snippet(script: &str) -> String {
        ...
        {
            match script {
                "printf 'hello'" => "echo|set /p=hello",
                "printf 'ok'" => "echo|set /p=ok",
                "printf 'oops' >&2; exit 7" => "echo oops 1>&2 & exit /b 7",
                "sleep 1" => "ping -n 2 127.0.0.1 > NUL",
                "true" => "echo background > NUL",
                _ => script,
            }
            .to_string()
        }
    ...
    fn web_search_handles_generic_links_and_invalid_base_url() {
        ...
        let error = {
            let _base_url = EnvVarGuard::set("CLAWD_WEB_SEARCH_BASE_URL", "://bad-base-url");
            execute_tool("WebSearch", &json!({ "query": "generic links" }))
    ...
    #[test]
    fn test_server_reads_long_request_heads() {
        let server = TestServer::spawn(Arc::new(|request_line: &str| {
            assert!(request_line.contains("GET /long-header "));
            HttpResponse::text(200, "OK", "long header handled")
        }));
        let mut stream = TcpStream::connect(server.addr()).expect("connect test server");
        let long_header = "x".repeat(8 * 1024);
        write!(
            stream,
        ...
        .expect("write long request");
        let mut response = String::new();
        stream
            .read_to_string(&mut response)
            .expect("read long request response");
        assert!(response.contains("long header handled"));
    }

    #[test]
    fn test_server_drop_does_not_hang_on_incomplete_request_head() {
        let server = TestServer::spawn(Arc::new(|_| {
            HttpResponse::text(200, "OK", "should not be reached")
        }));
        let mut stream = TcpStream::connect(server.addr()).expect("connect test server");
        write!(
            stream,
        ...
        assert!(
            ...
            "server drop should not wait forever for incomplete request headers"
        );
        drop(stream);
    }
  ...
  "nbformat_minor": 5
    ...
    fn bash_tool_reports_success_exit_failure_timeout_and_background() {
        let success = execute_tool(
            "bash",
            &json!({ "command": shell_snippet("printf 'hello'") }),
        )
        ...
        let failure = execute_tool(
            "bash",
            &json!({ "command": shell_snippet("printf 'oops' >&2; exit 7") }),
        )
        ...
        let timeout = execute_tool(
            "bash",
            &json!({ "command": shell_snippet("sleep 1"), "timeout": 10 }),
        )
        ...
        let background = execute_tool(
            "bash",
            &json!({ "command": shell_snippet("true"), "run_in_background": true }),
        )
...
"#;
    ...
    fn given_no_enforcer_when_bash_then_executes_normally() {
        ...
        let result = registry
            .execute("bash", &json!({ "command": shell_snippet("printf 'ok'") }))
                ...
                match listener.accept() {
                    Ok((mut stream, _)) => {
                        ...
                        let request = match read_http_request_head(&mut stream) {
                            ...
                            Err(error) => panic!("read request head failed: {error}"),
                        };
    ...
    fn read_http_request_head(stream: &mut TcpStream) -> std::io::Result<String> {
        ...
        loop {
            let size = stream.read(&mut scratch)?;
            if size == 0 {
                break;
            }
            request.extend_from_slice(&scratch[..size]);
