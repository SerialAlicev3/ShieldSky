...
mod tests {
    ...
    fn plugin_script_name(name: &str) -> String {
        if cfg!(windows) {
            format!("{name}.cmd")
        } else {
            format!("{name}.sh")
  ...
  "commands": [
    ...
    fn write_tool_plugin_with_name(root: &Path, name: &str, version: &str, tool_name: &str) {
        let script_path = root.join("tools").join("echo-json.py");
        write_file(
            &script_path,
            r#"import json
import os
...
payload = {
    "plugin": os.environ["CLAWD_PLUGIN_ID"],
    "tool": os.environ["CLAWD_TOOL_NAME"],
