...
mod tests {
    ...
    fn shell_snippet(command: &str) -> String {
        ...
        {
            match command {
                "printf 'blocked by plugin'; exit 2" => "echo blocked by plugin & exit /b 2",
                "printf 'broken plugin hook'; exit 1" => "echo broken plugin hook & exit /b 1",
                "printf 'later plugin hook'" => "echo later plugin hook",
                _ => command,
            }
            .to_string()
        }
    ...
    fn hook_script_name(base: &str) -> String {
        if cfg!(windows) {
            format!("{base}.cmd")
        } else {
            format!("{base}.sh")
    ...
    fn write_message_hook(path: &Path, message: &str) {
        #[cfg(windows)]
    ...
    fn write_hook_plugin(
        ...
        write_message_hook(&root.join("hooks").join(&pre_script), pre_message);
        write_message_hook(&root.join("hooks").join(&post_script), post_message);
        write_message_hook(&root.join("hooks").join(&failure_script), failure_message);
    ...
    fn pre_tool_use_denies_when_plugin_hook_exits_two() {
        ...
        let runner = HookRunner::new(crate::PluginHooks {
            pre_tool_use: vec![shell_snippet("printf 'blocked by plugin'; exit 2")],
    ...
    fn propagates_plugin_hook_failures() {
        ...
        let runner = HookRunner::new(crate::PluginHooks {
            pre_tool_use: vec![
                shell_snippet("printf 'broken plugin hook'; exit 1"),
                shell_snippet("printf 'later plugin hook'"),
            ],
            post_tool_use: Vec::new(),
            post_tool_use_failure: Vec::new(),
        });
