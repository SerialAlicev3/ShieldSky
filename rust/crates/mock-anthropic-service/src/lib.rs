    ...
    match scenario {
        ...
        Scenario::BashStdoutRoundtrip => match latest_tool_result(request) {
            ...
            None => tool_use_sse(
                "toolu_bash_stdout",
                "bash",
                &[&bash_tool_use_json("alpha from bash", 1000)],
            ),
        },
        Scenario::BashPermissionPromptApproved => match latest_tool_result(request) {
            ...
            None => tool_use_sse(
                ...
                "bash",
                &[&bash_tool_use_json("approved via prompt", 1000)],
            ),
        },
    ...
    match scenario {
        ...
        Scenario::BashStdoutRoundtrip => match latest_tool_result(request) {
            ...
            None => tool_message_response(
                ...
                "toolu_bash_stdout",
                "bash",
                json!({"command": bash_stdout_command("alpha from bash"), "timeout": 1000}),
            ),
        },
        Scenario::BashPermissionPromptApproved => match latest_tool_result(request) {
            Some((tool_output, is_error)) => {
            ...
            None => tool_message_response(
                ...
                "toolu_bash_prompt_allow",
                "bash",
                json!({"command": bash_stdout_command("approved via prompt"), "timeout": 1000}),
            ),
        },
        Scenario::BashPermissionPromptDenied => match latest_tool_result(request) {
            Some((tool_output, _)) => text_message_response(
...
fn bash_stdout_command(text: &str) -> String {
    if cfg!(windows) {
...
fn bash_tool_use_json(text: &str, timeout: u64) -> String {
    json!({
        "command": bash_stdout_command(text),
        "timeout": timeout,
    })
    .to_string()
}
